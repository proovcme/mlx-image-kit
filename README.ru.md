...
Работа разбита на этапы: кодирование prompts и освобождение text encoder; загрузка transformer, денойзинг и освобождение; загрузка VAE, декодирование и сохранение PNG. В batch промежуточные массивы временно сохраняются на диск, а каждый тяжёлый компонент загружается один раз. Поведение загрузчика проверяется в [`tests/test_loader.py`](tests/test_loader.py) без скачивания весов.

## Локальные данные и результаты

Сгенерированные PNG, веса, диагностические файлы, `local/` и `.history/` игнорируются Git. CLI хранит историю завершённых prompts локально для `/repeat`; она не входит в репозиторий. Шесть отобранных изображений в `assets/` — единственные сгенерированные картинки, предназначенные для публичного README. Метаданные PNG пусты: исходные prompts и приватные пути туда не записаны.

Тесты CLI, кеша и загрузчика проходят без загрузки весов:
