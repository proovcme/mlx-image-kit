...
В терминале показывается один индикатор денойзинга и сводка измерений после сохранения PNG. `--quiet` выводит в stdout только пути готовых PNG; ошибки идут в stderr. `--verbose` показывает источник модели и подробности загрузки. При перенаправлении вывода progress записывается несколькими обычными строками.
...
Для скриптов batch `--quiet` выводит в stdout по одному пути сохранённого PNG на строку; ошибки идут в stderr. Batch `--verbose` показывает источник модели и подробности загрузки.
